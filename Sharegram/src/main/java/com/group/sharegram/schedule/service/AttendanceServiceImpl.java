...
//		System.out.println("서비스 empNo "+ empNo);
//		System.out.println("서비스 attendance "+ attendance);
		...
		if (attendance == 1 && attCheck.get("attStatus").contains("null")) {
			...
			AttendanceDTO attendanceDTO = AttendanceDTO.builder().empNo(empNo).attStart(attStart).attStatus(attStatus)
					.build();
			...
			AttendanceDTO attendanceDTO = AttendanceDTO.builder().empNo(empNo).attStart(attCheck.get("attStart"))
	...
	public ResponseEntity<Object> getAttendanceList(int page) {
		...
		for (int i = 0; i < endTime.size(); i++) {
			...
			if (status.get(i).equals("정상 출근") && early.get(i).equals("")) {
				...
				//System.out.println("null? 지각 조퇴시 ");
				working.add(Long.toString(hour) + "시 " + Long.toString(minute) + "분");
