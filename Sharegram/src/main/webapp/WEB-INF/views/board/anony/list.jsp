	...
	<div class="anonybrd">
		<div class="brd_anony">
			<div class="anonyHeader">
				<div class="anony_title">
					<h2><strong>익명 게시판</strong></h2>
						<br>
						<div align="center" class="anony_msg">
							임직원 여러분의 의견을 자유롭게 표현하는 공간입니다. <br>
							사내 게시판 규정 운영원칙에 어긋나는 게시글은 삭제 될 수 있습니다.
						</div>
				</div>
...
<%-- 미로그인 시, 작성      <c:if test="${loginUser != null }">     합친 후 수정--%>
								<button id="btn_write" class="btn_primary" style='float:right;' onclick="location.href='${contextPath}/board/anony/write'">게시글작성</button>
<%--                        </c:if> --%>
								&nbsp;&nbsp;
								<button id="btn_first" class="btn_primary" style='float:right;' onclick="location.href='${contextPath}/board/anonyList'">첫페이지로</button>
				...
				<div class="anonyList">
					<table class="anonyBoard" align="center" id="anonyBoardList">
						...
						<tbody class ="brd_tbody" id="result">
							<c:if test="${ empty anonyBoardList }">
	                        		<tr>
	                        			<td colspan="4">등록된 게시글이 없습니다.</td>
	                        		</tr>
	                        	</c:if>
	                        	<c:forEach var="a" items="${anonyBoardList}">
				                        <tr>	<!-- background 수정 예정  -->
				                            <td class="anonyNo">${ a.anonyNo }</td>
				                            <td height="5%">
												<a href="${contextPath}/board/anony/detail?anonyNo=${ a.anonyNo }">${ a.anonyTitle }</a> <!-- 작성자는 조회수 증가x -->
											</td>
				                            <td height="5%"><input type="hidden" value="${ a.empNo }">익명</td>
				                            <td height="5%">${ a.createDate }</td>
				                            <%-- <td height="5%">${ a.hit }</td> --%>
				                        </tr>
				                </c:forEach>   
	                    </tbody>
	     				...
	     				<tfoot>
							<tr>
								<td colspan="4" id="paging" align="center">
									<hr>
				...
				<div style="margin:auto;text-align:center;" class="searchArea">
					<form style="display:inline-block;" id="frm_search" action="${contextPath}/board/anony/search">
						<select id="column" name="column" class="select">
							<option value="" align="center">::선택::</option>
							<option value="ANONY_TITLE">제목</option>
	...
	$(function(){
		...
		function fn_anonyList(){
			...
			$.ajax({	
				type : 'get',
				url : '${contextPath}/board/anonyList',
				dataType: 'json',
				data : 'pageNo=' + page ,
				success: function(resData){
					...
					var str = '' ;	
			 	    if(resData.beginPage != 1){
						str += '<a href=${contextPath}/board/anonyList?pageNo=' + Number(resData.beginPage -1) + '>' + '◀' + '</a>';
					}  
				 	for(let p = resData.beginPage; p <= resData.endPage; p++){
					 	if(p == resData.page){
							...
							str += '<a href=${contextPath}/board/anonyList?pageNo=' + p +'>'+ p + '</a>';
						} 
					} 
					if(resData.endPage != resData.totalPage) {
						str += '<a href=${contextPath}/board/anonyList?pageNo=' + Number(resData.endPage + 1) + '>' + '▶' + '</a>';
						//console.log(str);
					}     
	...
	function fn_searchList(){
		$('#search_area').click(function (){
			$.ajax({
				type : 'get',
				url : '${contextPath}/board/anony/autoComplete',
...
/* 					$.each(resData.anonyList, function( i , anonyList){
						...
						.append( $('<td class="content_line center_font">').text(anonyList.empNo) )
						.append( $('<td class="content_line">').append( $('<a>').text(anonyList.boardTitle).attr('href' ,'${contextPath}/board/anony/incrase/hit?anonyNo=' + anonyList.anonyNo ) ) )
						.append( $('<td class="content_line center_font">').text(anonyList.createDate) )
	...
	$(function(){
		...
		$('#btn_first').click(function (){
			location.href = '${contextPath}/board/anonyList';
		});
	})
	...
	$(function(){	
		...
		$('#recordPerPage').change(function(){
			location.href = '${contextPath}/board/anonyList?recordPerPage=' + $(this).val();
		});
	...
	// 게시글 전체 선택
   	$("#anonyBoardList").on("click", "#checkAll", function(){
