...
<body>
	...
	<div class="container_div" style="width:1580px;">
		<input type="hidden" name="apNo" value="${apNo.apNo}">
		<div id="apaside">
			<ul class="menu">
				<li class="menu_list"><a
					href="${pageContext.request.contextPath}/approval/driftlist"
		...
		<div class="page_section">
			...
			<div class="main_section">
				...
				<table class="board_apL">
					...
					<tr>
						<td>기안부서</td>
						<td><c:choose>
								<c:when test="${apNo.deptNo eq '10' }">인사총무부</c:when>
								<c:when test="${apNo.deptNo eq '11' }">재무회계부</c:when>
								<c:when test="${apNo.deptNo eq '12' }">영업부</c:when>
							</c:choose></td>
					</tr>
					<tr>
						<td>기안자</td>
					...
					<tr>
						<td>결재 상태</td>
						<td><c:choose>
								<c:when test="${apNo.apstate eq '0' }">결재대기</c:when>
								<c:when test="${apNo.apstate eq '1' }">결재완료</c:when>
								<c:when test="${apNo.apstate eq '2' }">반려사유 : ${apNo.aprejection }</c:when>
							</c:choose></td>
					</tr>
				</table>
				...
				<table class="board_apR">
					...
					<tr>
						<td style="padding: 3px 50px; text-align: center;">${apNo.jobName}</td>
					</tr>
					<tr>
						<td style="padding: 30px 0; text-align: center;"><c:if test="${apNo.apstate eq '1' }">${apNo.apperson }</c:if></td>
					</tr>
					<tr>
			...
			<c:if test="${loginEmp.empNo ==  apNo.appEmpNo}">
				<div class="apwrite">
					...
					<div id="newpjModal_a" class="ssb-modal">
						...
						<div class="ssb-modal-content">
							...
							<form action="${pageContext.request.contextPath}/approval/updateaprejection" method="get">
								<input type="hidden" name="apNo" value="${apNo.apNo}">
								<div id="pname">
