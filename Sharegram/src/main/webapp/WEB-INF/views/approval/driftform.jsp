...
<body>
	...
	<div class="container_div" style="width:1580px;">
		...
		<form action="${pageContext.request.contextPath}/approval/appinsert"
					...
					<div class="section_a">
						<span class="text">문서형식</span> <select class="opt_bt"
							...
							<c:forEach items="${ceo}" var="c">
								<option value="${c.empNo}">${c.name}</option>
							</c:forEach>
						</select>
					</div>

					<div class="section_a">
						<span class="text">기안부서 </span> <select class="opt_bt" name="deptNo">
							<option value="선택하세요">선택하세요</option>
							<c:forEach items="${deptList}" var="d">
								<option value="${d.deptNo}">${d.deptName}</option>
							</c:forEach>
						</select> <span class="text_a">기안자</span> <input type="text"
							class="opt_bt_a" name="apdrafter" value="${loginEmp.name}" readonly> <span class="text_a">기안
...
btn2.onclick = function(event) {
	location.href = '${pageContext.request.contextPath}/appproval/driftlist'
}
}
