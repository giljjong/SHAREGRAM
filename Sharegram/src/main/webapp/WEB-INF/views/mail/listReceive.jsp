...
<script>
	...
	function fn_getMailList(){
		...
		$.ajax({
			...
			success : function(resData){
				...
				.append(mail.empName == null ? $('<td class="name_tr">').html(mail.sender) : $('<td>').html(mail.empName))
				.append($('<td class="subject_tr" style="text-align:left;">').html(mail.readCheck == 'N' ? '<a class="detail_text"><strong>' + mail.subject + '</strong></a>' : '<a class="detail_text">' + mail.subject + '</a>'))
	...
	function fn_readEvent(){
		$(document).on('click', '.detail_text', function(event){
			var mailNo = $(this).parent().prev().prev().prev().prev().prev().children().first().val();
			fn_readMail(mailNo);
		});	// onClick
	}	// fn
...
<body>
	<div class="mail_body_wrap" style="padding-left:65px;">
		...
		<div style="min-height:800px;">
			<div>
				<div class="cnt_info">
					<span class="title_text">받은메일함</span>
				...
				<div class="btn_wrap_content">
						...
						<div><button class="btn_toggle mail_btn"><span class="delete text">삭제</span></button></div>
