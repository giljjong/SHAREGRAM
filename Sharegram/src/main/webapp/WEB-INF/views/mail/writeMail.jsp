...
<script>
	...
	function fn_sendToToggle(){
		...
		$(document).on('click', '#sendMe', function(event){
			...
			$('#btn_toMe').show();
			$('.from_toggle').hide();
			$('.btn_wrap_content').css('justify-content', 'flex-start');
			$('.cc_box').hide();
			$('#strTo').val($('#from').val());
		});
		...
		$(document).on('click', '#sendOther', function(event){
			...
			$('#btn_toMe').hide();
			$('#strTo').val('');
			$('.btn_wrap_content').css('justify-content', 'space-between');
			$('.from_toggle').show();
			$('.cc_box').show();
		});
	}
...
<body>
	<div class="mail_body_wrap" style="padding-left:65px;">
		<div class="mail_body">
			<span class="title_text">메일쓰기</span>
			<br>
			
			<hr>
			
			<div class="btn_wrap_content" >
					<input type="button" class="btn_send from_toggle mail_btn" value="보내기">
					<input type="button" class="from_toggle mail_btn" value="임시저장">
					<input type="button" class="from_toggle mail_btn" value="미리보기">
					<input type="button" class="btn_send mail_btn" id="btn_toMe" value="저장">
					<input type="button" id="sendMe" class="mail_btn" value="내게쓰기">
					<input type="button" id="sendOther" class="mail_btn" value="메일쓰기" style="margin-left : 20px;">
			</div>
			...
			<form id="frm_send" method="post" enctype="multipart/form-data">
				...
				<div class="from_toggle">
					<label for="strTo">받는사람</label>
					<input type="text" name="strTo" id="strTo" class="input_box" value="${email}"><br>
				</div>
				
				<div class="cc_box">
					<label for="strCc">참조</label>
					<input type="text" name="strCc" class="input_box" id="strCc"><br>
				</div>
				
				<label for="subject">제목</label>
				<input type="text" name="subject" class="input_box" id="subject"><br>
				
				<div class="insert">
			    	<span>첨부파일</span><label for="files"><span class="attach_btn">내 PC</span></label>
				...
				<script>
					...
					function fn_submit(){
						$('.btn_send').click(function(){
							...
							var form = document.querySelector("#frm_send");
